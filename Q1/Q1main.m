...
% A5000 = Matgeneration(matrix_size(5), seed);
A = {A20, A100, A500, A1000};
...
figure;
for m = 1:num_methods
    plot(matrix_size(m), times(:, m), '-o', 'DisplayName', methods{m});
    hold on;
end
...
figure;
for m = 1:num_methods
    plot(matrix_size(m), iterations(:, m), '-o', 'DisplayName', methods{m});
    hold on;
end
...
figure;
for m = 1:num_methods
    plot(matrix_size(m), errors(:, m), '-o', 'DisplayName', methods{m});
    hold on;
end
